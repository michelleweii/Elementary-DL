...
# 文件数据的生成用到了PTB2.py文件
TRAIN_DATA = "PTB/generate/ptb.train"  # 训练数据路径
EVAL_DATA = "PTB/generate/ptb.valid"
TEST_DATA = "PTB/generate/ptb.test"
...
In iteration: 1
After 0 steps, perplexity is 10083.871
'''
...
# 输出的矩阵比输入数据多一个维度，新增维度的大小是EMB_SIZE。在语言模型中，一般input_data的维度是batch_size*num_steps，
# 2️而输出的input_embedding的维度是batch_size*num_steps*EMB_SIZE.
...
# 从logits计算log perplexity作为损失函数：
# labels是一个大小为[batch_size*num_steps]的以为数组，它包含每个位置正确的单词编号。
# logits的维度是[]
