...
# 在这个过程中将用到dropout方法，dropout可以进一步提升模型可靠性并防止过拟合，dropout过程只能在训练时使用。
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope('layer1-conv1'):
        conv1_weights = tf.get_variable('weight',[CONV1_SIZE,CONV1_SIZE,
                                                  NUM_CHANNELS,CONV1_DEEP],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    # 实现第二层池化层的前向传播过程。这里选用最大池化层，池化层过滤器的边长为2，使用全0填充且移动的步长为2。
        # 这一层的输入时上一层的输出，也就是28*28*32的矩阵。输出为14*14*32的矩阵。
    with tf.name_scope('layer2-pool1'):
        pool1 = tf.nn.max_pool(relu1,ksize=[1,2,2,1],padding='SAME')
    ...
    #     pool2.get_shape函数可以得到第四层输出矩阵的维度而不需要手工计算。注意因为每一层神经网络的输入输出都为一个batch的矩阵，
